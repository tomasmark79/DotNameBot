...
namespace dotnamebot::utils {
  ...
  std::shared_ptr<IAssetManager>
    ...
    return dotnamebot::assets::AssetManagerFactory::createDefault(executablePath, appName);
  }
  ...
  std::shared_ptr<ILogger> UtilsFactory::createLogger(LoggerType type, const LoggerConfig &config) {
    return dotnamebot::logging::LoggerFactory::create(type, config);
  }

  std::shared_ptr<ILogger> UtilsFactory::createDefaultLogger() {
    LoggerConfig config{.level = dotnamebot::logging::Level::LOG_INFO,
