...
class ConsoleLogger : public dotnamebot::logging::ILogger {
private:
  std::mutex logMutex_;
 ...
 */
#ifdef DEBUG
  dotnamebot::logging::Level currentLevel_ = dotnamebot::logging::Level::LOG_DEBUG;
#else
  dotnamebot::logging::Level currentLevel_ = dotnamebot::logging::Level::LOG_INFO;
#endif
...
public:
   ...
   * @param caller
   */
  void log(dotnamebot::logging::Level level, const std::string &message,
  ...
  void setLevel(dotnamebot::logging::Level level) override {
    std::lock_guard<std::mutex> lock(logMutex_);
    currentLevel_ = level;
  };

  [[nodiscard]]
  dotnamebot::logging::Level getLevel() const override {
   ...
   * @return std::string
   */
  static std::string levelToString(dotnamebot::logging::Level level) {
    switch (level) {
    case dotnamebot::logging::Level::LOG_DEBUG: return "DBG";
    case dotnamebot::logging::Level::LOG_INFO: return "INF";
    case dotnamebot::logging::Level::LOG_WARNING: return "WRN";
    case dotnamebot::logging::Level::LOG_ERROR: return "ERR";
    case dotnamebot::logging::Level::LOG_CRITICAL: return "CRI";
    default: return "INF";
    }
  }
...
// no colors, no reset
...
#ifdef _WIN32
  static void setConsoleColorWindows(dotnamebot::logging::Level level) {
    const std::map<dotnamebot::logging::Level, WORD> colorMap = {
        {dotnamebot::logging::Level::LOG_DEBUG,
         FOREGROUND_BLUE | FOREGROUND_GREEN | FOREGROUND_INTENSITY},
        {dotnamebot::logging::Level::LOG_INFO, FOREGROUND_GREEN | FOREGROUND_INTENSITY},
        {dotnamebot::logging::Level::LOG_WARNING,
         FOREGROUND_RED | FOREGROUND_GREEN | FOREGROUND_INTENSITY},
        {dotnamebot::logging::Level::LOG_ERROR, FOREGROUND_RED | FOREGROUND_INTENSITY},
        { dotnamebot::logging::Level::LOG_CRITICAL,
...
#else
  static void setConsoleColorUnix(dotnamebot::logging::Level level) {
    static const std::map<dotnamebot::logging::Level, const char *> colorMap = {
        {dotnamebot::logging::Level::LOG_DEBUG, "\033[34m"},
        {dotnamebot::logging::Level::LOG_INFO, "\033[32m"},
        {dotnamebot::logging::Level::LOG_WARNING, "\033[33m"},
        {dotnamebot::logging::Level::LOG_ERROR, "\033[31m"},
        {dotnamebot::logging::Level::LOG_CRITICAL, "\033[95m"}};
    auto it = colorMap.find(level);
    if (it != colorMap.end()) {
      std::cout << it->second;
    } else {
  ...
  static void setConsoleColor(dotnamebot::logging::Level level) {
#ifdef _WIN32
...
private:
   ...
   * @param now_tm
   */
  void logToStream(std::ostream &stream, dotnamebot::logging::Level level,
  ...
  std::string buildHeader(const std::tm &now_tm, const std::string &caller,
                          dotnamebot::logging::Level level) const {
    std::ostringstream header;
    if (includeName_) {
      header << "[" << headerName_ << "] ";
    }
